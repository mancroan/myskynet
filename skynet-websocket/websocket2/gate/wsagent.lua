...
function operate.on_open(fd)
	print("open===========")
end

function operate.on_message(fd, text, opcode)
	print("message===========", text, fd)
	if GameID then
		socket.write(GameID, text)
	else
		print("gameserver not conn =====")
	end
end

function operate.on_close(code, reason)
	print("close===========", reason)
end
...
local function _dispatch(fd, text, opcode)
	...
	if opcode == CLOSE then
	    ...
	    operate.on_close(code, reason)
	end
end

local function _recv(fd)
	local last
	local frames = {}
	local first_opcode
	...
	while true do
		...
		repeat
			...
			if decoded then
				...
				encoded = rest
				if fin == true then
					if not _dispatch(fd, table.concat(frames), first_opcode) then
...
function agent.gamedata(fd, data)
	_send(data)
end

function agent.start(fd, gameid)
