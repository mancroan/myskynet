...
#define SOCKET_TYPE_INVALID 0   //初始的socket
#define SOCKET_TYPE_RESERVE 1   //申请了id
#define SOCKET_TYPE_PLISTEN 2   //加入监听前
#define SOCKET_TYPE_LISTEN 3    //已监听
#define SOCKET_TYPE_CONNECTING 4 //正在连接
#define SOCKET_TYPE_CONNECTED 5  //已连接
#define SOCKET_TYPE_HALFCLOSE 6   
#define SOCKET_TYPE_PACCEPT 7   // accept前
#define SOCKET_TYPE_BIND 8      //bind
...
//socket的结构体
struct socket {
	uintptr_t opaque;  //消息的来源
	struct wb_list high;  
	struct wb_list low;
	int64_t wb_size;  
	int fd;
	int id;      //一个不重复的id
	uint16_t protocol; //协议项
	uint16_t type;    //O,S, L
	union {
		int size;
		uint8_t udp_address[UDP_ADDRESS_SIZE];
...
struct socket_server {
	...
	int checkctrl;    //检查是否有命令
	poll_fd event_fd;  //服务器文件描述符
	int alloc_id;      //一个id 不重复
	int event_n;       //事件个数
	int event_index;
	struct socket_object_interface soi;
	struct event ev[MAX_EVENT]; //事件集 64个
	struct socket slot[MAX_SOCKET]; //socket队列  65535个
	char buffer[MAX_INFO];  //buffer 128字节
...
/*
	...
	S Start socket		  //创建socket
	B Bind socket		  //绑定
	L Listen socket		  //监听
	K Close socket        //关闭
	O Connect to (Open)   //打开一个连接
	X Exit				  //退出
	D Send package (high) //发送数据包
	P Send package (low)  //发送数据包
	A Send UDP package    //发送UDP数据包
	T Set opt             //设置操作
	U Create UDP socket   //创建udp socket
	C set udp address     //设置udp 地址
 */
...
reserve_id(struct socket_server *ss) {
	...
	for (i=0;i<MAX_SOCKET;i++) {
		int id = __sync_add_and_fetch(&(ss->alloc_id), 1);   //原子操作 自加1
		if (id < 0) {
			id = __sync_and_and_fetch(&(ss->alloc_id), 0x7fffffff);
		}
		struct socket *s = &ss->slot[HASH_ID(id)];  //取出socket
		if (s->type == SOCKET_TYPE_INVALID) {
			//如果s->type == SOCKET_TYPE_INVALID  s->type = SOCKET_TYPE_RESERVE 返回true
...
socket_server_create() {
	int i;
	int fd[2];
	poll_fd efd = sp_create();  //创建epoll的句柄
	if (sp_invalid(efd)) {  	//判断句柄是否非法
		fprintf(stderr, "socket-server: create event pool failed.\n");
		return NULL;
	}
	if (pipe(fd)) { 		  	//创建管道
		sp_release(efd); 		//关闭epoll句柄
		fprintf(stderr, "socket-server: create socket pair failed.\n");
		return NULL;
	}
	if (sp_add(efd, fd[0], NULL)) { //将读管道句柄的事件加入epoll管理
	...
	ss->recvctrl_fd = fd[0]; //接收管道句柄
	ss->sendctrl_fd = fd[1]; //写管道句柄
	ss->checkctrl = 1;		//

	for (i=0;i<MAX_SOCKET;i++) {  //初始化socket队列
		struct socket *s = &ss->slot[i];
		s->type = SOCKET_TYPE_INVALID;  //一开始都是0 准备阶段
		clear_wb_list(&s->high);
		clear_wb_list(&s->low);
	}
	ss->alloc_id = 0;  //一个id  每次申请会id = alloc_id+1 
	ss->event_n = 0;   //发生的事件数
	ss->event_index = 0; //事件的处理的下标
	memset(&ss->soi, 0, sizeof(ss->soi));
	FD_ZERO(&ss->rfds);
	assert(ss->recvctrl_fd < FD_SETSIZE);  //必须保证接收管道句柄小于系统句柄数
	return ss;
}
...
start_socket(struct socket_server *ss, struct request_start *request, struct socket_message *result) {
	...
	if (s->type == SOCKET_TYPE_PACCEPT || s->type == SOCKET_TYPE_PLISTEN) {
		if (sp_add(ss->event_fd, s->fd, s)) { //把fd加入到监听
			s->type = SOCKET_TYPE_INVALID;
			return SOCKET_ERROR;
		}
...
ctrl_cmd(struct socket_server *ss, struct socket_message *result) {
	...
	uint8_t buffer[256];
	uint8_t header[2];
	block_readpipe(fd, header, sizeof(header));  //读出头部
	int type = header[0];  
	int len = header[1];
...
socket_server_poll(struct socket_server *ss, struct socket_message * result, int * more) {
	for (;;) {
		...
		if (ss->event_index == ss->event_n) {
			ss->event_n = sp_wait(ss->event_fd, ss->ev, MAX_EVENT); //取出事件
			ss->checkctrl = 1;
			if (more) { //没用的
				*more = 0;
			}
...
send_request(struct socket_server *ss, struct request_package *request, char type, int len) {
	request->header[6] = (uint8_t)type;   //操做类型
	request->header[7] = (uint8_t)len;  //请求类型
	for (;;) {
		int n = write(ss->sendctrl_fd, &request->header[6], len+2); //向管道发送类容
		if (n<0) {
			if (errno != EINTR) {
...
open_request(struct socket_server *ss, struct request_package *req, uintptr_t opaque, const char *addr, int port) {
	...
	if (id < 0)
		return -1;
	req->u.open.opaque = opaque;  //请求的发起者标识
	req->u.open.id = id; //id
...
do_bind(const char *host, int port, int protocol, int *family) {
	...
	if (protocol == IPPROTO_TCP) {
		...
		ai_hints.ai_socktype = SOCK_DGRAM; //udp
	}
	...
	*family = ai_list->ai_family;
	...
	if (setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, (void *)&reuse, sizeof(int))==-1) { //REUSEADDR
		goto _failed;
	}
	status = bind(fd, (struct sockaddr *)ai_list->ai_addr, ai_list->ai_addrlen); //绑定
	if (status != 0)
		goto _failed;
...
socket_server_listen(struct socket_server *ss, uintptr_t opaque, const char * addr, int port, int backlog) {
	int fd = do_listen(addr, port, backlog); //创建socket, bind，listen
	if (fd < 0) {
		return -1;
	}
	struct request_package request;
	int id = reserve_id(ss); //申请一个唯一id
	if (id < 0) {
		close(fd);
		return id;
	}
	...
	request.u.listen.fd = fd;  //fd 是server fd
